...
**Corrida diaria desde el 10/09** (tarea programada de Windows "JEREN Indexacion CIMAT", 09:05, `tec-harness/jeren_indexacion.ps1` → `tec-harness/output/jeren/indexacion_cimat.md`). Primera corrida, la noche del 10/09: Google ya había releído el sitemap y **4 de 7 URLs estaban indexadas** (`/cimat`, cardanes, ventiladores, normas); turbos, especificaciones y aplicaciones "descubiertas, sin indexar".

**Regla de validación (17/09/2026):** mirar la tabla diaria o correr `python tec-harness/src/gsc_jeren.py`. Esperado: `/cimat`, `/cimat/balanceadora-de-cardanes-y-ciguenales`, `/cimat/balanceadora-de-ventiladores-e-impulsores` y `/cimat/balanceadora-de-turbos` en estado "Enviada e indexada", y el sitemap con fecha de lectura posterior al 10/09. Si alguna sigue en "Google no reconoce esta URL", el pedido no alcanzó: revisar en el mismo resultado si hay error de rastreo o canonical distinta, y volver a pedir la indexación. Segunda validación el 01/10: al menos una consulta orgánica con "cardan" o "cigüeñal" sobre la subpágina de cardanes en el informe de consultas.
...
**Aprendizaje del monitor, 14/09 08:30.** (1) **Las negativas no bloquean la variante con tilde.** `alineacion y balanceo` está negada en frase desde el 10/09 y `equipo de alineación y balanceo` se sirvió el 12/09 y el 13/09 con un clic cada vez (ARS 2.333); `balanceo dinámico de ventiladores` pasó con `[balanceo dinamico de ventiladores]` negada. Las positivas sí matchean con y sin tilde. Solo 5 de las 203 negativas tienen tilde: toda negativa con vocal acentuada o ñ/ü se carga en las dos formas. (2) La discrepancia WhatsApp del 11/09 (GA4 2 `whatsapp_click` cpc, Ads 0) se explica por fecha: la etiqueta "Ads - Conversión CIMAT WhatsApp" entró en la versión 4 de GTM publicada el 10/09 a las 15:07, y los clics fueron a las 13 h y a las 15 h. Label y acción coinciden; la etiqueta sigue sin prueba en vivo porque no hubo ningún `whatsapp_click` después. (3) `maquina equilibradora` en frase, a cuatro días de creada: 65% del gasto post-cambio y 8 de 10 impresiones visibles de gomería; "equilibradora" trae ruedas, no máquina. Propuesta de pausa en el parte del 14/09.

**14/09, aprobado por Ariel (propuestas 1 y 2 del parte 2026-09-14; la 3 no).** (1) Pausar `maquina equilibradora` en frase (criterion `206537216064~339038388216`), sin reemplazo. Baseline 11–14/09: 39 impr, 5 clics, ARS 5.421 (65% del gasto post-cambio), 8 de 10 impresiones visibles de gomería. (2) 22 negativas con tilde en `NEG — Universal CIMAT` (17 en frase, 5 en exacta; lista en `tec-harness/src/jeren_ads_aplicar_20260914.py`), 203 → 225. Baseline: 3 impr y 2 clics (ARS 2.333) en formas con tilde de negativas existentes. **Umbral, 24/09:** gasto visible en instrumentos/ruedas/damper = 0; 0 fugas en la sección "Fugas por tilde" de `jeren_ads_estado.py` (control fijo agregado el 14/09); CPC promedio < 1.000 sobre ≥ 25 clics. Reversión: reactivar la frase / quitar las 22. **Estado: APLICADO el 14/09 a las 09:51 por API** (`jeren_ads_aplicar_20260914.py`), verificado leyendo de vuelta: keyword `PAUSED`, lista con 225 miembros, ninguna negativa faltante. Los dos intentos anteriores de esa mañana los bloqueó el modo automático de la sesión; salió al desactivarlo.
