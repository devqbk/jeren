...
import { aireRefrigeracionContent } from "@/lib/brands-content"

export async function generateStaticParams() {
  return aireRefrigeracionBrands.map((brand) => ({
    slug: brand.slug,
  }))
}
...
export default async function BrandPage({
  ...
  const content = aireRefrigeracionContent.find((c) => c.slug === slug)
  const otherBrands = aireRefrigeracionBrands.filter((b) => b.slug !== slug)
          ...
          <div className="mx-auto max-w-7xl px-4 lg:px-8">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbLink href="/aire-acondicionado">Aire y Refrigeración</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
        ...
        <section className="border-t border-border py-12">
          <div className="mx-auto max-w-7xl px-4 lg:px-8">
            <h3 className="mb-6 text-center text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              Otras marcas de Aire y Refrigeración
            </h3>
            <div className="flex flex-wrap items-center justify-center gap-3">
              {otherBrands.map((b) => (
                <Link
                  key={b.slug}
                  href={`/aire-acondicionado/${b.slug}`}
                  className="rounded-md border border-border bg-card px-4 py-2.5 text-sm font-medium text-foreground transition-all hover:border-primary/40 hover:text-primary"
                >
                  {b.name}
                </Link>
              ))}
