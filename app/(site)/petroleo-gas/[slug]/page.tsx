...
export async function generateStaticParams() {
  ...
  const petroleoBrands = marcasNuevas.filter(
    (b) => b.industrias?.includes("petroleo-gas") && !b.landing
  )
  return petroleoBrands.map((brand) => ({
    slug: brand.slug,
  }))
}
...
export async function generateMetadata({
  ...
  const brand = marcasNuevas.find((b) => b.slug === slug && b.industrias?.includes("petroleo-gas"))
...
export default async function BrandPage({
  ...
  const petroleoBrands = marcasNuevas.filter((b) => b.industrias?.includes("petroleo-gas"))
  const otherBrands = petroleoBrands.filter((b) => b.slug !== slug)
          ...
          <div className="mx-auto max-w-7xl px-4 lg:px-8">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbLink href="/petroleo-gas">Petróleo y Gas</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
          ...
          <section className="border-t border-border py-12">
            <div className="mx-auto max-w-7xl px-4 lg:px-8">
              <h3 className="mb-6 text-center text-sm font-semibold uppercase tracking-wider text-muted-foreground">
                Otras marcas de Petróleo y Gas
              </h3>
              <div className="flex flex-wrap items-center justify-center gap-3">
                {otherBrands.map((b) => (
                  <Link
                    key={b.slug}
                    href={urlDeMarca("/petroleo-gas", b)}
